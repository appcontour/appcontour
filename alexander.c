...
alexander (struct presentation *p)
{
  ...
  switch (rank)
  {
    ...
    printout_ideal2 (0, determinant2, extradeterminants, extradets, deficiency);
    break;
...
alexander_fromideal (struct alexanderideal *ai)
{
  ...
  switch (ai->indets)
  {
    ...
    case 1:
      printf ("Case with %d indeterminates not yet implemented\n", ai->indets);
    break;
                     ...
                     struct laurentpoly2 **fundamentals, int fnum, int deficiency)
{
  ...
  assert (ai == 0);
    ...
    if (fnum > 0)
    {
      ...
      if (deficiency == 2)
      {
        print_laurentpoly2 (principal, 'u', 'v');
        printf (";\n");
      }
      for (j = 0; j < fnum; j++)
      {
