...
printout_ideal1 (struct alexanderideal *ai, struct laurentpoly *principal) 
{
  ...
  if (ai && ai->l1num > 1)
  {
    if (outformat == OUTFORMAT_APPCONTOUR) printf ("ideal(t) {\n");
                     ...
                     struct laurentpoly **fundamentals, int fnum, int printprincipal)
{
  ...
  {
    if (outformat == OUTFORMAT_APPCONTOUR) printf ("ideal(u,v%s) {\n", extraindets);
    if (!quiet) printf ("Alexander ideal generated by:\n");
    if (quiet && outformat != OUTFORMAT_APPCONTOUR) printf ("Ideal [\n");
    if (printprincipal || principal || (ai && ai->l2num > 0))
    {
      if (principal || printprincipal) {print_laurentpoly (principal, "uvwxyz"); printf (";\n");}
      if (ai)
      {
