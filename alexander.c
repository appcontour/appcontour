  ...
  switch (ai->indets)
  {
    ...
    case 2:
      if (verbose)
      {
        ...
        {
          print_laurentpoly (ai->lx[i].l2, "uv");
          printf (";\n");
        }
        for (i = 0; i < ai->fl2num; i++)
        {
          printf ("F: ");
          print_laurentpoly (ai->lx[i + ai->fl2offset].l2, "uv");
          printf (";\n");
        }
        printf ("}\n");
      }
                     ...
                     struct laurentpolyx **fundamentals, int fnum, int printprincipal)
{
  ...
  if (fnum > 0 || (ai && ai->fl2num > 0) || (ai && ai->l2num >= 2))
  {
    ...
    {
      if (principal || printprincipal) {print_laurentpoly (principal, "uv"); printf (";\n");}
      if (ai)
      {
        for (j = 0; j < ai->l2num; j++)
        {
          print_laurentpoly (ai->lx[j].l2, "uv");
          printf (";\n");
        }
      }
    }
    for (j = 0; j < fnum; j++)
    {
      ...
      {
        printf ("F: ");
        print_laurentpoly (fundamentals[j], "uv");
        printf (";\n");
      } else {
        printf ("(");
        print_laurentpoly (fundamentals[j], "uv");
    ...
    if (ai)
    {
      for (j = 0; j < ai->fl2num; j++)
      {
        ...
        {
          printf ("F: ");
          print_laurentpoly (ai->lx[j + ai->fl2offset].l2, "uv");
          printf (";\n");
        } else {
          printf ("(");
          print_laurentpoly (ai->lx[j + ai->fl2offset].l2, "uv");
    ...
    if (ai == 0) print_laurentpoly (principal, "uv");
     else {
      assert (ai->l2num == 1);
      print_laurentpoly (ai->lx[0].l2, "uv");
    }
    printf (";\n");
  }
  if (outformat == OUTFORMAT_APPCONTOUR) printf ("}\n");
    ...
    switch (rank)
    {
      ...
      for (j = 0; j < matrix->numcols; j++)
      {
        ...
        for (i = 0; i < matrix->numrows - 1; i++)
        {
          ...
          ai->lx[ai->l2num++].l2 = laurent_dup(l);
        }
      }
      ...
      for (j = 0; j < matrix->numcols; j++)
      {
        ...
        ai->lx[ai->fl2num++ + ai->fl2offset].l2 = laurent_dup(l);
      }
...
laurent_get_exp_sum (struct presentationrule *r, int n, int gconj)
{
  ...
  l = (struct laurentpolyx *) malloc (POLYSIZE(stemdegree + 1));
  l->indets = 1;
...
laurent_common_factorx (struct presentationrule *r, int x1, int x2)
{
  ...
  if (poly2)
  {
    ...
    uminus1 = (struct laurentpolyx *) malloc (POLYSIZE(2));
    uminus1->indets = 1;
    uminus1->minexpon = 0;
...
laurent_compute_determinant (struct laurentpolyx ***matrix, int n)
{
  ...
  if (n == 0)
  {
    determinant = (struct laurentpolyx *) malloc (POLYSIZE(1));
...
laurent_compute_determinantx (struct laurentpolyx ***matrix, int n)
{
  ...
  if (n == 0)
  {
    ...
    determinant->minexpon = 0;
    determinant->stemdegree = 0;
    lp1 = (struct laurentpolyx *) malloc (POLYSIZE(1));
...
base_canonifyx2_onedim (struct laurentpolyx **ppt)
{
  ...
  newp1 = (struct laurentpolyx *) malloc (POLYSIZE(mygcd+1));
  newp1->indets = 1;
