...
alexander (struct presentation *p)
{
  ...
  switch (rank)
  {
    ...
    if (extradeterminants)
    {
      ...
      for (j = 0; j < numcols; j++) laurent_canonify2 (extradeterminants[j]);
             ...
             struct laurentpoly2 ***extradeterminantspt)
{
  ...
  extrarow = (struct laurentpoly2 **) malloc (numcols*sizeof(struct laurentpoly2 *));
  for (j = 0; j < numcols; j++)
  {
  ...
  for (j = 0, r = p->rules; r; j++, r = r->next)
  {
    ...
    extrarow[j] = laurent_common_factor2 (r, e1, e2);
    if (extrarow[j]) extrafound++;
  }
  ...
  if (debug)
  {
    for (j = 1, r = p->rules; r; j++, r = r->next)
    {
      ...
      print_laurentpoly2 (extrarow[j-1], 'u', 'v');
      printf ("\n");
    }
  }
  ...
  if (verbose)
  {
    ...
    printf ("Extra row with commutator factors:\n");
    for (j = 0; j < numcols; j++)
    {
      print_laurentpoly2 (extrarow[j], 'u', 'v');
      printf ("; \t");
    }
    printf ("\n");
  }
  ...
  if (extrafound && extradeterminantspt)
  {
    *extradeterminantspt = extradeterminants =
    ...
    for (j = 0; j < numcols; j++)
    {
      extradeterminants[j] = laurent_minor_determinant2 (matrix, numcols, extrarow, j);
    }
  }
...
struct laurentpoly2 *
laurent_minor_determinant2 (struct laurentpoly2 ***matrix, int n,
                            struct laurentpoly2 **extrarow, int row_to_subst)
{
  int j;
  struct laurentpoly2 **matrixcolumn;
  ...
  for (j = 0; j < n; j++)
  {
    ...
    matrixcolumn[row_to_subst] = extrarow[j];
    extrarow[j] = saved;
  }

  determinant = laurent_compute_determinant2 (matrix, n);
